use crate::mock::{
    new_test_ext, AgentStaking, Balances, IdentityCore, OnboardingDistribution, RuntimeEvent,
...
fn agent_ref(
    byte: u8,
) -> ContentRef<<Test as crate::Config>::MaxReasonCidLen, <Test as crate::Config>::MaxReasonUriLen>
{
...
fn root_can_bond_and_unbond_registered_agent() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Balances::balance_on_hold(&crate::HoldReason::AgentStake.into(), &1),
            100
        );

        assert_ok!(AgentStaking::request_unbond(
            RuntimeOrigin::signed(1),
            identity_id,
            agent_id,
            40
        ...
        assert_ok!(AgentStaking::release_unbond(
            RuntimeOrigin::signed(1),
            identity_id,
            agent_id
        ));
...
fn release_can_be_blocked_and_cleared() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(AgentStaking::block_release(
            RuntimeOrigin::root(),
            identity_id,
            agent_id,
            None
        ...
        assert_ok!(AgentStaking::clear_release_block(
            RuntimeOrigin::root(),
            identity_id,
            agent_id
        ));
